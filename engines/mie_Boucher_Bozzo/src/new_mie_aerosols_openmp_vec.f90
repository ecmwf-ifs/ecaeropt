...
!                                     refactor in subroutines for more structured code.
!------------------------------------------------------------------------------------------------
!
...
!----------------------------------------------------------------------------------------------
    ...
    INTEGER(4) , PARAMETER :: Nbin_max=999
    ...
    REAL(8)    :: tot_num_conc, radius, radius2, mass
    ...
    REAL(KIND=JPRB) , DIMENSION(0:Nbin_max) :: vg, vr, vx,vdeltar, vnumb, vomega
    REAL(KIND=JPRB) , DIMENSION(0:Nbin_max) :: vQ_sca, vQ_abs, vQ_ext, vmass
    REAL(KIND=JPRB) , DIMENSION(Nmumax, 0:Nbin_max) :: vphase, vPP
      ...
      DO Nwv=1,nb_lambda
         ...
         if(rmin.eq.rmax) Nbin_points=0
            ...
            if (verbose==3.and.Nwv==1) then
              ...
              call mie_core_an_bn(x, m, Nmax, a, b, Nsize)
              call mie_core_qvals(x, m, Nmax, a, b, Nsize, Q_ext, Q_sca, Q_abs, omega, g)
         ...
         DO dbin=0, Nbin_points 
            ...
            IF(rmin.eq.rmax) then
               ...
               vr(dbin)      = logn_r( rmin, rmax, dbin, Nbin_points)
               vdeltar(dbin) = logn_dr(rmin, rmax, Nbin_points)
               vnumb(dbin)   = logn_nt(Ntot, sigma_g, vr(dbin), r_0, pi, Ndis, Nbin_points, dbin)
            ...
            vmass(dbin)=logn_mass(rh_int, cutoff_radius, vnumb(dbin), vdeltar(dbin), rho, vr(dbin), rh_growth(IRH), pi)

            !size parameter (assuming refr. index in vacuum)
            vx(dbin)=2._JPRB*pi*vr(dbin)/lambda_out(Nwv)

            call mie_core_an_bn(vx(dbin), m, Nmax, a, b, Nsize)
            call mie_core_qvals(vx(dbin), m, Nmax, a, b, Nsize, vQ_ext(dbin), vQ_sca(dbin), vQ_abs(dbin), vomega(dbin), vg(dbin))
            call mie_core_phase(Nsize, Nmax, Nmumax, vx(dbin), a, b, ph_ang, vQ_sca(dbin), vphase(:,dbin), pi)

            
            !-- for computation of LR:  phase function / lidar ratio -----------
            DO Nmu=1, Nmumax 
               vPP(Nmu,dbin)=vQ_sca(dbin)*vphase(Nmu,dbin)*vnumb(dbin)*(vr(dbin)**2)*vdeltar(dbin)
            ENDDO
         ...
         vQ_ext       = vQ_abs + vQ_sca
         tot_num_conc = sum(vnumb(:)*vdeltar(:))
         radius       = sum(vr(:)*vnumb(:)*vdeltar(:))
         radius2      = sum(vr(:)*vr(:)*vnumb(:)*vdeltar(:))
         volume       = 4._JPRB/3._JPRB*pi*sum((vr(:)**3._JPRB)*vnumb(:)*vdeltar(:))
         surface      = 4._JPRB*pi*sum((vr(:)**2._JPRB)*vnumb(:)*vdeltar(:))
         rgv          = sum(log(vr(:))*(vr(:)**3._JPRB)*vnumb(:)*vdeltar(:))
         rgn          = sum(log(vr(:))*vnumb(:)*vdeltar(:))
         sigma_sca    = pi*sum((vr(:)**2)*vQ_sca(:)) 
         sigma_ext    = pi*sum((vr(:)**2)*vQ_ext(:))
         sigma_abs    = pi*sum((vr(:)**2)*vQ_abs(:))
         gtot         = pi*sum((vr(:)**2)*vQ_sca(:)*vnumb(:)*vdeltar(:)*vg(:))/sigma_sca
         omega        = pi*sum((vr(:)**2)*vQ_sca(:)*vnumb(:)*vdeltar(:)*vomega(:))/sigma_ext
         mass         = sum(vmass)
         PP           = sum(vPP, dim=2)*pi/sigma_sca

         !-- for LR
         rgv=exp(rgv/(volume*3./4./pi))
         rgn=exp(rgn/tot_num_conc)
         radius =radius  /tot_num_conc
         radius2=radius2/tot_num_conc
...
contains
         ...
         real(8) function logn_mass(rh_int, cutoff_radius, numb, deltar, rho, r, rh_growth, pi)
            ...
            subroutine mie_core_qvals(x, m, Nmax, a, b, Nsize, Qext, Qsca, Qabs, omega, g)
            ...
            subroutine mie_core_an_bn(x, m, Nmax, a, b, Nsize)
