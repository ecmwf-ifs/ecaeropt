...
!----------------------------------------------------------------------------------------------
   ...
   !   ---------------------
    ...
    !print *, "                    OpenMP Fortran with n. threads: ", OMP_get_num_thread()
...
DO bin=1,size_bins
   ...
   DO IRH=1,rh_int
      ...
      DO Nwv=1,nb_lambda
         ...
         ! this can be a subroutine which calculate optical properties for a given
         ! wl between rmin and rmax.
         !
         !
         ! subroutine mie_core(wl, rmin, rmax, others)
         ...
         DO dbin=0, Nbin_points 
            ...
            IF(rmin.eq.rmax) then
               ...
               numb=0._JPRB
               DO dis=1, Ndis
                  numberdis=logn_nt(Ntot(dis), sigma_g(dis),r, r_0(dis), pi)
...
contains
         ...
         real(8) function logn_nt(Ntot, sigma_g, r, r_0, pi)
             implicit none
             real(8)    , INTENT(IN)  :: r, r_0, sigma_g, pi, Ntot

             logn_nt=Ntot/SQRT(2._JPRB*pi)/log(sigma_g)*exp(-0.5_JPRB*(log(r/r_0)/log(sigma_g))**2)
